jQuery(document).ready(function($) {
  ...
  value = value.replace(/ /gi, '|');
   $("#available-posts div").each(function () {
    if ($(this).attr('title').search(new RegExp(value, "i")) < 0) { $(this).fadeOut('fast'); } else { $(this).fadeIn('fast'); }
   });
 })
	...
	$('.cpr_button').click(function(){ 
		...
		$.ajax({
			...
			data: { action: 'cpr-cats', catID: $('#cpr_filters').val(), postID: $('#h_pid').val(), howMany: $('#howmany').val(), orderBy: $('#orderby').val(), orderIn: $('#orderin').val()  },
			...
			success: function(response){ 
					...
					$($duplicates).find('a').removeClass('addme').addClass('added').html('Added');								
				}//success	
		});//ajax		
    ...
    if ( $("#related-posts").find( "#" + postId ).size() ) return false;
    post.clone().fadeIn('normal').appendTo('#related-posts').find('a.addme').html('Remove').removeClass('addme').addClass('removeme').siblings('input').attr('name','reladded[]');
    $(this).removeClass('addme').addClass('added').html('Added');
			return false;
	});	
	...
	$('.removeme').live('click', function() {
			var post = $(this).parents('div.thepost');
   var postId = post.attr("id");
			post.fadeOut('normal', function() {$(this).remove(); });			
			$('#available-posts div#' + postId).find('a').removeClass('added').addClass('addme').html('Add');	
			return false;
	});
